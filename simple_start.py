#!/usr/bin/env python3
"""
Simple start script without Unicode issues
"""
import os
import sys
import subprocess
...
def create_server_file():
    """Create server file without Unicode characters"""
...
"""
    ...
    # Write with UTF-8 encoding to handle any Unicode
    with open("hospitrackserver.py", "w", encoding="utf-8") as f:
        f.write(server_code)
...
def install_minimal_packages():
    ...
    try:
        ...
        print("Package installation failed, but server might still work")
...
def start_server():
    ...
    try:
        subprocess.run([sys.executable, "hospitrackserver.py"])
